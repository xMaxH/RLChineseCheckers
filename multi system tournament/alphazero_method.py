...
from az.encoder import decode_action, encode_action, encode_legal_mask, encode_state, slot_of
...
def _load_deploy() -> Dict[str, Any]:
    """Optional deploy overrides (written by tools/overnight.py): lets the
    tournament mode + MCTS sims/shaping be set without editing this file.
    Absent file => unchanged legacy behaviour (heuristic_rollout, 200 sims)."""
    try:
        here = os.path.dirname(os.path.abspath(__file__))
        with open(os.path.join(here, "runs", "mcts_deploy.json")) as f:
            return json.load(f)
    except Exception:
        return {}
...
_MCTS_CFG = MCTSConfig(
    n_sim=int(_DEPLOY.get("n_sim", 200)),
    batch_leaves=32, dirichlet_alpha=0.0, dirichlet_eps=0.0,
    virtual_loss=1.0, c_puct=float(_DEPLOY.get("c_puct", 1.5)),
    shaping_enabled=bool(_DEPLOY.get("shaping", False)),
    shaping_scale=0.15, shaping_goal_weight=0.5,
)
# mcts_pool mode: AlphaZero MCTS restricted to the heuristic candidate pool,
# RL-trained value network evaluating leaves. No hand-crafted rollouts.
_MCTS_POOL_CFG = MCTSConfig(
    n_sim=int(_DEPLOY.get("n_sim", 300)),
    batch_leaves=32, dirichlet_alpha=0.0, dirichlet_eps=0.0,
    virtual_loss=1.0, c_puct=float(_DEPLOY.get("c_puct", 1.5)),
    shaping_enabled=True, shaping_scale=0.15, shaping_goal_weight=0.5,
    restrict_to_pool=True,
    restrict_pool_root_only=bool(_DEPLOY.get("root_only", False)),
)
...
def _choose_heuristic_pool_rollout(
    ...
    # Defaults tuned 2026-05-18: top_k 3->5 widens the rollout-checked candidate
    # set. Measured worst case (6p) ~1.1s/move — safely inside the 2s budget.
    top_k = max(1, int(os.getenv("ALPHAZERO_ROLLOUT_TOPK", "5")))
...
# ---------------------------------------------------------------------------
# RL value-network move selection.  The hand heuristic *proposes* a small pool
# of candidate moves (the role AlphaGo's policy network played); the RL-trained
# value network *evaluates* each candidate and selects the move.  This is a
# value-based RL agent assisted by a heuristic prior — and because the pool is
# always strong heuristic moves, a value head that is weak in 5-6p games cannot
# wander into bad moves the way free MCTS did.
# ---------------------------------------------------------------------------
_SHAPE_SCALE = 0.15
_SHAPE_GOAL_W = 0.5


def _value_of_move(sim: Sim, my_colour: str, pid: int, to_idx: int) -> float:
    """Score a candidate move with the RL-trained value network: apply it on a
    copy and evaluate the resulting position from my_colour's view.  The value
    head was trained with potential-based shaping (it predicts E[T+Phi]-Phi(s)),
    so Phi(s') is added back to recover the unbiased expected outcome."""
    s = copy.deepcopy(sim)
    s.apply_move(pid, to_idx)
    if s.is_terminal:
        return 100.0 if s.winner == my_colour else -100.0
    to_move = s.current_colour()
    board, glob = encode_state(s.pins_state(), to_move, s.turn_order, s.move_count)
    _, values = _NN_EVAL(board[None], glob[None])
    v = float(values[0][slot_of(my_colour, to_move)])
    return v + potential(s, my_colour, _SHAPE_SCALE, _SHAPE_GOAL_W)


def _rank_pool_by_value(sim: Sim, my_colour: str,
                        legal_moves: Dict[int, List[int]]):
    """Heuristic proposes the move pool; the RL value network ranks it.
    Returns [(value, pid, to_idx), ...] best first."""
    pool = heuristic_move_pool(sim, my_colour, legal_moves)
    scored = [(_value_of_move(sim, my_colour, pid, to_idx), pid, to_idx)
              for _, _, pid, to_idx in pool]
    scored.sort(key=lambda x: -x[0])
    return scored


def _choose_value_pool(sim: Sim, my_colour: str,
                       legal_moves: Dict[int, List[int]]) -> Tuple[int, int]:
    """Value-RL selection: heuristic proposes, the RL value network picks."""
    scored = _rank_pool_by_value(sim, my_colour, legal_moves)
    if not scored:
        return heuristic_choose_move(sim, my_colour, legal_moves)
    return scored[0][1], scored[0][2]


def _choose_value_rollout(sim: Sim, my_colour: str,
                          legal_moves: Dict[int, List[int]]) -> Tuple[int, int]:
    """The RL value network ranks the heuristic pool; short heuristic rollouts
    confirm the top few — AlphaGo combined a value network with rollouts."""
    scored = _rank_pool_by_value(sim, my_colour, legal_moves)
    if not scored:
        return heuristic_choose_move(sim, my_colour, legal_moves)
    if len(scored) == 1:
        return scored[0][1], scored[0][2]
    top_k = max(1, int(os.getenv("ALPHAZERO_VALUE_TOPK", _DEPLOY.get("value_topk", 3))))
    rollouts = max(1, int(os.getenv("ALPHAZERO_ROLLOUTS_PER_MOVE",
                                    _DEPLOY.get("rollouts_per_move", 2))))
    rollout_cap = max(1, int(os.getenv("ALPHAZERO_ROLLOUT_MAX_MOVES",
                                       _DEPLOY.get("rollout_max_moves", 500))))
    best_key, best_move = None, (scored[0][1], scored[0][2])
    for rank, (_, pid, to_idx) in enumerate(scored[:top_k]):
        margins = []
        for ri in range(rollouts):
            s = copy.deepcopy(sim)
            s.apply_move(pid, to_idx)
            rr = random.Random(_rollout_seed(sim, my_colour, pid, to_idx, ri))
            _finish_with_heuristic(s, rr, max_moves=rollout_cap)
            margins.append(_score_margin(s, my_colour))
        key = (float(np.mean(margins)), -rank)
        if best_key is None or key > best_key:
            best_key, best_move = key, (pid, to_idx)
    return best_move
...
def _choose_mcts_pool(
    sim: Sim,
    my_colour: str,
    legal_moves: Dict[int, List[int]],
) -> Tuple[int, int]:
    """AlphaZero MCTS restricted to the heuristic candidate-move pool, with the
    RL-trained value network evaluating leaves. No hand-crafted rollouts decide
    the move -- the search is the decision. The heuristic only bounds the
    action set (the role AlphaGo's policy network played), which also fixes the
    multi-player wandering that unrestricted MCTS suffered."""
    visits, _, _ = run_search(sim, _NN_EVAL, _MCTS_POOL_CFG, add_dirichlet_at_root=False)
    if int(visits.sum()) == 0:
        return heuristic_choose_move(sim, my_colour, legal_moves)
    legal_action_set = {
        encode_action(pid, to_idx, my_colour)
...
def choose_move_alphazero(
    ...
    mode = os.getenv("ALPHAZERO_POLICY_MODE",
                     _DEPLOY.get("mode", "heuristic_rollout")).strip().lower()
    ...
    elif mode in ("mcts_pool", "mcts-pool", "pool"):
        pid, to = _choose_mcts_pool(sim, my_colour, server_legal)
    elif mode in ("value_pool", "value-pool", "value"):
        pid, to = _choose_value_pool(sim, my_colour, server_legal)
    elif mode in ("value_rollout", "value-rollout"):
        pid, to = _choose_value_rollout(sim, my_colour, server_legal)
    elif mode == "heuristic":
