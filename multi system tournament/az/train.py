...
def train_one_stage(
    ...
    auto_bootstrap: bool = True,  # safety fallback for cold starts
    may10_ckpt: Optional[str] = None,   # frozen opponent model (environment only)
    shaping_enabled: bool = False,      # potential-based progress shaping
    shaping_scale: float = 0.15,
    shaping_goal_weight: float = 0.5,
    terminal_weight: float = 1.0,
    value_head_only: bool = False,      # RL-train only the value head
    policy_anchor: bool = False,        # distill policy to frozen seed; RL trunk+value
):
    ...
    run_meta = {
        ...
        "may10_ckpt": may10_ckpt,
        "shaping": {
            "enabled": shaping_enabled,
            "scale": shaping_scale,
            "goal_weight": shaping_goal_weight,
    ...
    # Value-network RL: freeze the trunk + policy head so the MCTS policy prior
    # stays byte-identical to the seed model; RL-train ONLY the value head from
    # self-play game outcomes. AlphaZero policy iteration degrades a strong
    # supervised policy here (search at our sim budget produces visit targets
    # flatter than the BC policy), so we hold the policy fixed and let RL
    # improve only the value estimate. BatchNorm is kept in eval mode in the
    # train loop below so the frozen trunk's outputs cannot drift.
    if value_head_only:
        for p in net.parameters():
            p.requires_grad = False
        for p in list(net.val_fc1.parameters()) + list(net.val_fc2.parameters()):
            p.requires_grad = True
        n_train = sum(p.numel() for p in net.parameters() if p.requires_grad)
        n_total = sum(p.numel() for p in net.parameters())
        print(f"[train] value-head-only: training {n_train}/{n_total} params "
              f"(val_fc1+val_fc2); trunk + policy head frozen")
    ...
    # Policy-anchor value RL: keep a frozen copy of the seed model. The trunk
    # and value head are RL-trained on real self-play outcomes, while the policy
    # head is distilled toward this frozen seed policy every step (pi target :=
    # ref policy). This lets the value network genuinely learn (the trunk can
    # adapt) without the AlphaZero policy-iteration collapse, since the MCTS
    # prior is held at the strong supervised policy instead of chasing the
    # flatter visit-count targets.
    ref_net = None
    if policy_anchor:
        ref_net = AZNet().to(device)
        ref_net.load_state_dict(net.state_dict())
        ref_net.eval()
        for p in ref_net.parameters():
            p.requires_grad = False
        print("[train] policy-anchor: policy distilled to frozen seed; "
              "trunk + value head RL-trained on game outcomes")

    candidate_nn_eval = make_nn_eval(net, device)
    snapshot_nets: List[AZNet] = []
    snapshot_evals = []

    # Frozen May-10 opponent (environment only — never a policy target).
    # Single-process self-play uses this eval directly; workers load their own.
    may10_nn_eval = None
    if may10_ckpt is not None and os.path.exists(may10_ckpt):
        may10_nn_eval = make_nn_eval(load_model(may10_ckpt, device), device)
        print(f"[may10] loaded frozen opponent from {may10_ckpt}")
    elif may10_ckpt is not None:
        print(f"[may10] WARNING: ckpt not found: {may10_ckpt} — running without it")
    ...
    mcts_cfg = MCTSConfig(
        ...
        shaping_goal_weight=shaping_goal_weight,
        terminal_weight=terminal_weight,
    )
    ...
    plog(f"  bootstrap_chunks={bootstrap_chunks}  max_chunks={max_chunks}  "
         ...
         f"auto_bootstrap={auto_bootstrap}")
    ...
    plog(f"  shaping: enabled={shaping_enabled} scale={shaping_scale} "
         f"goal_weight={shaping_goal_weight} terminal_weight={terminal_weight}  "
         f"may10_ckpt={may10_ckpt} may10_frac={sp_cfg.may10_slot_frac}")
    plog("=" * 72)
    ...
    if num_workers > 0:
        ...
        pool = WorkerPool(
            ...
            device=str(device), seed=rng_seed, may10_ckpt=may10_ckpt,
        )
    ...
    # Baseline eval BEFORE any training. For a warm-started run this locks the
    # seed checkpoint's win-rate in as the best.pt floor: the eval gate below
    # can then only ever replace best.pt with a strictly stronger model, so a
    # degrading run can no longer overwrite a good artifact with a worse one.
    best_winrate = -1.0
    if seed_ckpt is not None:
        net.eval()
        plog(f"baseline EVAL of seed_ckpt ({train_cfg.eval_games} games)...")
        base_eval = eval_vs_heuristic(
            candidate_nn_eval,
            num_games=train_cfg.eval_games,
            num_players=stage.player_counts[0],
            mcts_cfg=mcts_cfg,
            rng=rng,
        )
        health.record_eval(base_eval)
        best_winrate = base_eval["win_rate"]
        torch.save(net.state_dict(), os.path.join(out_dir, "best.pt"))
        stage_result_path = os.path.join(out_dir, "best.pt")
        plog(f"baseline win_rate={best_winrate:.3f} "
             f"margin={base_eval['score_margin_mean']:.0f} -> best.pt floor locked")
    ...
    while chunk_idx < max_chunks:
        ...
        # Bootstrap if explicitly requested, or if the cold-start safety fallback
        # is enabled and MCTS is not yet producing trainable games. Warm-started
        # RL fine-tuning should pass auto_bootstrap=False so bootstrap_chunks=0
        # really means the first chunk is neural-network-guided MCTS self-play.
        requested_bootstrap = chunk_idx < bootstrap_chunks
        fallback_bootstrap = auto_bootstrap and (
            (not has_trained_at_least_once and len(replay) < train_cfg.min_samples_to_train)
            or consecutive_empty_mcts_chunks >= 2
        )
        bootstrap = requested_bootstrap or fallback_bootstrap
        ...
        if pool is not None:
            ...
            # Shaping cold start IS MCTS (Phi-only leaves) — it needs the model.
            worker_needs_model = (not bootstrap) or bootstrap_dagger or shaping_enabled
            ...
            for g in range(sp_cfg.games_per_chunk):
                ...
                result = play_one_game(
                    ...
                    selfplay_cfg=sp_cfg,
                    rng=rng,
                    may10_nn_eval=(None if (bootstrap and not shaping_enabled)
                                    else may10_nn_eval),
        ...
        if len(replay) >= train_cfg.min_samples_to_train:
            ...
            # value_head_only keeps the whole net in eval mode: BatchNorm uses
            # frozen running stats so the trunk (and thus the policy) cannot
            # drift; the value head's Linear layers train identically either way.
            if not value_head_only:
                net.train()
            ...
            for _ in range(n_steps):
                ...
                if policy_anchor:
                    # pi target := frozen seed policy (distillation anchor), so
                    # the policy stays at the strong BC distribution while the
                    # value head + trunk RL-train on the game-outcome targets.
                    with torch.no_grad():
                        ref_logits, _ = ref_net(boards, globs)
                        pis = torch.softmax(ref_logits, dim=-1)
            ...
            if (not bootstrap) and (train_step - last_eval_step >= train_cfg.eval_every_steps):
                ...
                # Save best — eval-gated: best.pt only ever moves to a STRICTLY
                # stronger model, so a degrading run can never overwrite it.
                best_path = os.path.join(out_dir, "best.pt")
                if eval_metrics["win_rate"] > best_winrate:
                    best_winrate = eval_metrics["win_rate"]
                    torch.save(net.state_dict(), best_path)
                    stage_result_path = best_path
                    plog(f"  chunk {chunk_idx} NEW BEST "
                         f"win_rate={best_winrate:.3f} -> {best_path}")
                # Stage pass = early stop once win-rate clears the bar.
