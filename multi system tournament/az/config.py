...
class MCTSConfig:
    ...
    # --- progress shaping (genuine-RL value targets; see shaping.py) ---
    # Defaults below = exact pre-shaping behaviour (flat +-1 terminal value).
    shaping_enabled: bool = False
    shaping_scale: float = 0.15        # max magnitude of the progress potential Phi
    shaping_goal_weight: float = 0.5   # pins-in-goal vs goal-distance weight inside Phi
    terminal_weight: float = 1.0       # magnitude of the real win/loss reward
    # Restrict the MCTS node's action set to the heuristic's candidate-move
    # pool. Keeps multi-player search from wandering into bad moves; the search
    # itself (PUCT + RL value net) is unchanged AlphaZero MCTS.
    restrict_to_pool: bool = False
    # If True, restrict only the root node (the move actually played stays
    # bounded to good moves, but deeper search skips the per-node pool cost --
    # much faster, so more simulations fit the 2s budget).
    restrict_pool_root_only: bool = False
...
class SelfPlayConfig:
    ...
    may10_slot_frac: float = 0.0   # frac of slots filled by the frozen May-10 opponent
