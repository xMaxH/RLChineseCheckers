...
def _worker_main(worker_id: int, task_q, result_q, init_kwargs: Dict[str, Any]):
    ...
    from az.inference_server import make_nn_eval, load_model
    ...
    # Frozen May-10 opponent: loaded once, never updated. Environment only.
    may10_state: Dict[str, Any] = {"eval": None, "loaded": False}

    def _ensure_may10():
        if not may10_state["loaded"]:
            may10_state["loaded"] = True
            ckpt = init_kwargs.get("may10_ckpt")
            if ckpt:
                may10_state["eval"] = make_nn_eval(load_model(ckpt, device), device)
        return may10_state["eval"]
        ...
        try:
            ...
            if op == "load_main":
                ...
                result = play_one_game(
                    ...
                    selfplay_cfg=sp_cfg,
                    rng=rng,
                    may10_nn_eval=(None if (task["bootstrap"] and not mcts_cfg.shaping_enabled)
                                   else _ensure_may10()),
