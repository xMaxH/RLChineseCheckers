...
from .encoder import (
    encode_state, encode_legal_mask, decode_action, slot_of,
)
from .sim import Sim
...
def _terminal_v_dict(sim: Sim) -> Dict[str, float]:
    """+1 for winner, -1 for everyone else. {} if max_moves (caller shouldn't hit this in MCTS)."""
    out = {}
    if sim.terminal_reason in ('WIN', 'DRAW_CHAIN') and sim.winner is not None:
        for c in sim.colours:
            out[c] = 1.0 if c == sim.winner else -1.0
    return out
...
def _expand_node(
    ...
    if n_legal == 0:
        ...
        node.terminal_v_dict = _terminal_v_dict(sim)
        return
...
def _select_leaf(
    ...
    if sim.is_terminal and not root.is_terminal:
        ...
        root.terminal_v_dict = _terminal_v_dict(sim)
        return [], root
    ...
    while node.is_expanded and not node.is_terminal:
        ...
        if child is None:
            ...
            if sim.is_terminal:
                ...
                child.terminal_v_dict = _terminal_v_dict(sim)
            else:
                child = MCTSNode(
                    parent=node, parent_idx=idx,
...
def _v_vec_to_dict(v_vec: np.ndarray, to_move: str, colours: List[str]) -> Dict[str, float]:
    """Convert a (6,) canonical-slot value vector to a {colour: value} dict using the leaf's to_move."""
    out: Dict[str, float] = {}
    for c in colours:
        s = slot_of(c, to_move)
        out[c] = float(v_vec[s])
...
def run_search(
    ...
    if work_sim.is_terminal:
        ...
        root.is_terminal = True
        root.terminal_v_dict = _terminal_v_dict(work_sim)
    ...
    while sims_done < cfg.n_sim:
        ...
        if batch_leaves:
            ...
            for i in range(B):
                ...
                v_dict = _v_vec_to_dict(vals[i], batch_leaves[i].to_move, batch_leaves[i].colours)
