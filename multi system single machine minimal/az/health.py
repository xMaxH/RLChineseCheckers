...
class HealthMonitor:
    ...
    def record_eval(self, metrics: Dict) -> None:
        ...
        unique = metrics.get("score_margin_unique", 0)
        if unique <= 3:
            self.eval_unique_streak_below += 1
            if self.eval_unique_streak_below >= 2:
                self.kill_reason = (
                    f"score_margin_unique <= 3 for {self.eval_unique_streak_below} evals (collapse?)"
                )
        else:
            self.eval_unique_streak_below = 0

        margin = metrics.get("score_margin_mean", 0.0)
        self.eval_margin_history.append(margin)
        if len(self.eval_margin_history) >= 3:
            last3 = list(self.eval_margin_history)[-3:]
            if last3[0] > last3[1] > last3[2] and last3[2] < 0:
                self.kill_reason = (
                    f"score_margin_mean trending negative: {last3}"
                )
